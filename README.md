...
| `void	ft_lstdelone(t_list *lst, void (*del)(void *))` | Frees node `lst` and its content |
| `void	ft_lstclear(t_list **lst, void (*del)(void *))` | Frees all nodes in `lst` and its content |
| `void	ft_lstiter(t_list *lst, void (*f)(void *))` | Applies `f` to all content nodes in `lst` |
| `t_list	*ft_lstmap(t_list *lst, void *(*f)(void *), void (*del)(void *))` | Applies `f` to all content nodes in `lst` and uses `del` if necessary |

### Other projects 
Other projects of 42 that I've added to the library for convenience
| **Function** | **Description** |
| :-------- | :--------- |
| `char	*get_next_line(int fd)` | Program that reads `fd` given |
| `int	ft_printf(char const *arg, ...)` | Dupe of printf function |
